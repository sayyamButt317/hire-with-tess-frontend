...
export default function MainLayout({ children, hideSignUp = false }: { children: React.ReactNode, hideSignUp?: boolean }) {
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-r from-orange-400 to-pink-500">
      <header className="flex justify-between items-center p-6 text-white">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          {!hideSignUp && (
              <Link href="/signup" className="bg-tess-blue rrrr text-white px-4 py-2 rounded-md hover:bg-blue-700">
                Sign Up
              </Link>
            )}
          </nav>
